#!/usr/bin/env python3
"""
创建完整有效的 CiteTrack macOS Xcode 项目
包含所有源文件，修复后的代码
"""
...
print("🔨 创建 CiteTrack_macOS.xcodeproj...")
...
excluded = [
    ...
    'StatisticsView.swift',  # 与 ChartsViewController 中的定义冲突
    'ModernChartsViewController.swift',  # 与 ChartDataService 中的 ChartConfiguration.default 冲突
]
source_files = [f for f in all_sources if f not in excluded]

print(f"📝 包含 {len(source_files)} 个源文件")
print(f"🚫 排除 {len(excluded)} 个有冲突的文件")
...
# 添加源文件的 PBXBuildFile
for fname, ids in sorted(file_data.items()):
...
print(f"📁 包含 {len(source_files)} 个源文件 + CoreData 模型")
print(f"\n🎯 排除的文件:")
for f in excluded:
    print(f"  - {f}")
print("\n🚀 可以在 Xcode 中打开了：")
